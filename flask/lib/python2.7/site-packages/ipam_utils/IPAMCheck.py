...
class IPAMCheck:
	...
	def checkOnRouterandIPAM(self,hostname,sourceIPs):
	    ...
	    routerCheck=self.checkIPAMIP(hostname,sourceIPs,firstLP,fullIP)
	    ipamCheck=self.db.selectRecord(firstLP)
	    if ipamCheck == -1:
		log.info("%s and IPAM processing got failed ",str(routerCheck))
		return routerCheck+" and IPAM check got failed"
	    elif ipamCheck:
		...
		if "exact" in str(routerCheck).strip():
		    return routerCheck+" avilable in  IPAM"
		return routerCheck+" and avilable in  IPAM"
	    else:
		log.info("%s and not on New IPAM ",str(routerCheck))
		print routerCheck+" and not on New IPAM"
		return routerCheck+" and not in IPAM"
	    
	    
        def checkIPAMIP(self,hostname,sourceIPs,firstLP,fullIP):
	    Next=0
	    Previous=0
	    pHandler=""
	    pHostname=""
		...
		while 1:
			...
			if output:
				...
				if baseRouter == 0:
				    lPrefix=self.parsingShowBgpForPrefix(output,command,firstLP,fullIP)
				...
				if "ErrorParser" in str(cRouter):
					...
					if "ErrorParser" in str(vrfName):
					    ...
					    return  ipvrf+" is  on default VRF table"
					else:
					    log.info("%s IP  on %s VRF Routing Table",str(ipvrf),str(vrfName))
					    return  ipvrf+" is  on "+str(vrfName)+" vrf routing table."	
				    return rHandler
...
if __name__ == "__main__":
    ...
    lastIps=object.checkOnRouterandIPAM('10.10.10.70','200.12.221.125/32') 
